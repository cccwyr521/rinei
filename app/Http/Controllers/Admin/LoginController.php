...
class LoginController extends Controller
{
    ...
    public function sendEmail(Request $request)
    {
        ...
        if(empty($data)){
            ...
            try{
                ...
                $url = sprintf(env('APP_URL')."/admin/forget/reset?username=%s&email=%s&activeCode=%s",$username,$email,
                    ToolsEmail::createActiveCode($username ,$email));
                //发送html的邮件
                //视图的数据
                $viewData = [
    ...
    {      
        ...
        if(empty($data)){
            ...
            return redirect('/admin/forget/pwd')->with('msg','用户或邮箱不存在');
        } 
        ...
        if($activeCode != $params['activeCode'] ){
            ...
            return redirect('/admin/forget/pwd')->with('msg','激活码不存在');
        }
    ...
    public function save(Request $request)
    {
        ...
        $info = $this->storeData($object,['password' => md5($params['password']) ]);

        if(!$info){

             return redirect('/admin/forget/pwd')->with('msg','密码重置失败');
        }
